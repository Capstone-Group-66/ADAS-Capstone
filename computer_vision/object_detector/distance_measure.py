...
import numpy as np
import typing  
from object_detector.core import RectInfo
	...
	def DrawDetectedOnFrame(self, frame_show: cv2) -> None : 
		if ( len(self.distance_points) != 0 )  :
			for x, y, d in self.distance_points:
				...
				def putText_shadow(img, text, org, fontFace, fontScale, color, thickness, shadow_color=(50,50,50)):
					x, y = org
					cv2.putText(img, text, (x+1, y+1), fontFace, fontScale, shadow_color, thickness)
