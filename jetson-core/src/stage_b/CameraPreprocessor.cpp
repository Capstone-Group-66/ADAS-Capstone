...
CameraPreprocessor::CameraPreprocessor(Mount mount,
  ...
  if (!std::filesystem::exists(yaml_path)) {
    std::cerr << "[CameraPreprocessor] WARNING: Calibration not found for "
              << mount_name << " at " << yaml_path << "\n";
...
bool CameraPreprocessor::loadCalibration(const std::string &yaml_path) {
  ...
  try {
    ...
    if (intrinsics_.camera_matrix.empty() || intrinsics_.dist_coeffs.empty()) {
      std::cerr << "[CameraPreprocessor] ERROR: Invalid calibration data in "
                << yaml_path << "\n";
      return false;
    }

    if (intrinsics_.image_width <= 0 || intrinsics_.image_height <= 0) {
      std::cerr << "[CameraPreprocessor] ERROR: Invalid image dimensions in "
                << yaml_path << "\n";
      return false;
    }
...
void CameraPreprocessor::computeUndistortMaps() {
  ...
  std::cout << "[CameraPreprocessor] Undistort maps computed. ROI: "
...
CameraFrameData CameraPreprocessor::process(const CameraFrameData &input,
                                            bool crop_to_roi) {
  ...
  cv::Mat frame(input.height, input.width, CV_8UC3,
