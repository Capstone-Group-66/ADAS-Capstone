  ...
  while (running_.load(std::memory_order_relaxed)) {
    ...
    if (g_verbose_mode.load()) {
      ...
      auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(
                         now - last_verbose_time)
                         .count();
      if (elapsed >= 5) {
        std::cout << "\n[Stage B] " << mountToString(mount_) << ": "
                  << total_dets_in_window << " detections in "
    ...
    // Push to output queue (may drop if full - that's OK per "freshness over
    // completeness")
...
void StageBManager::start() {
  ...
  std::cout
      << "==============================================================\n";
  std::cout
      << "          STAGE B: CAMERA PREPROC + INFERENCE                 \n";
  std::cout
      << "==============================================================\n";
