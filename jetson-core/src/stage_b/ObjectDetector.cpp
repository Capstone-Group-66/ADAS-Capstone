...
void ObjectDetector::loadEngine() {
    ...
    for (int i = 0; i < nbBindings; i++) {
        ...
        std::cout << "[ObjectDetector] Binding " << i << " ("
                  << (engine_->bindingIsInput(i) ? "Input" : "Output") << ") Dims: ";
...
void ObjectDetector::allocateBuffers() {
    ...
    std::cout << "[ObjectDetector] CUDA buffers allocated (input: " << input_size_ / 1024
              << " KB, output: " << output_size_ / 1024 << " KB)\n";
}
...
std::vector<Detection> ObjectDetector::postprocess(float *output, const cv::Size &original_size) {
    ...
    for (int i = 0; i < num_detections; ++i) {
        ...
        // Filter: Only keep ADAS-relevant classes (person, bicycle, car, motorcycle, bus, truck)
        // COCO: 0=person, 1=bicycle, 2=car, 3=motorcycle, 5=bus, 7=truck
        if (max_class_id != 0 && max_class_id != 1 && max_class_id != 2 && max_class_id != 3 &&
            max_class_id != 5 && max_class_id != 7) {
            continue;
        }
    ...
    cv::dnn::NMSBoxes(boxes, confidences, config_.confidence_threshold, config_.nms_threshold,
                      indices);
