...
ObjectDetector::~ObjectDetector() { releaseBuffers(); }

void ObjectDetector::loadEngine() {
  std::cout << "[ObjectDetector] Loading TensorRT engine from "
            << config_.model_path << "...\n";

  // Read engine file
  std::ifstream file(config_.model_path, std::ios::binary | std::ios::ate);
  if (!file.is_open()) {
    std::cerr << "[ObjectDetector] ERROR: Cannot open engine file: "
              << config_.model_path << "\n";
    return;
  }
  ...
  for (int i = 0; i < nbBindings; i++) {
    ...
    std::cout << "[ObjectDetector] Binding " << i << " ("
...
void ObjectDetector::allocateBuffers() {
  ...
  input_size_ =
      1 * 3 * config_.input_width * config_.input_height * sizeof(float);
  ...
  std::cout << "[ObjectDetector] CUDA buffers allocated (input: "
            << input_size_ / 1024 << " KB, output: " << output_size_ / 1024
            << " KB)\n";
}
...
DetBatch ObjectDetector::detect(const cv::Mat &frame, const Header &header) {
  ...
  for (int c = 0; c < 3; ++c) {
    memcpy(input_data + c * channel_size, channels[c].data,
...
ObjectDetector::postprocess(float *output, const cv::Size &original_size) {
  ...
  float y_scale =
      static_cast<float>(original_size.height) / config_.input_height;
  ...
  for (int i = 0; i < num_detections; ++i) {
    ...
    // Filter: Only keep ADAS-relevant classes (person, bicycle, car,
    // motorcycle, bus, truck) COCO: 0=person, 1=bicycle, 2=car, 3=motorcycle,
    // 5=bus, 7=truck
...
ObjectDetector::ObjectDetector(const Config &config) : config_(config) {
  std::cout
      << "[ObjectDetector] STUB: TensorRT not available (non-ARM platform)\n";
  loaded_ = false;
}
