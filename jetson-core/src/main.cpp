  ...
  while (g_visualizer_running.load() && !g_shutdown_requested.load()) {
    ...
    if (got_frame && !batch.frame.empty()) {
      ...
      if (g_ble_server && g_ble_server->isConnected()) {
        ...
        // Fix: Include proximity_alert in alerting condition so Radar-only
        // alerts are sent
        ...
        if (is_alerting || time_since.count() > 1000) {
          ...
          if (fcw_alert.has_value()) {
            ...
            alert.type =
                adas::AlertType::FCW;  // Map to FCW for Mobile App (turns red)
...
void startPipeline(const adas::Config &config, const adas::HardwareMap &hw_map,
  ...
  // The FrontCam detections (DetBatch) will be fused with FrontRadar in Stage
  // E.
  ...
  // - SideCamL/R: Needed for Blind Spot Detection (BSD) and Lane Change Warning
  // (LCW)
      ...
      switch (choice) {
        case 1:  // Start Pipeline
          ...
          if (hw_map.mappings.empty()) {
            std::cout << "[Main] No devices mapped. Run Device Wizard first "
                         "(option 4)\n";
          } else {
            startPipeline(config, hw_map, calib_dir, model_path);
          }
        ...
        case 7:  // Test RTT to Pi4
        {
          ...
          if (rtt < 0) {
            ...
            if (rtt < 10) {
              ...
              std::cout
                  << "[RTT] Status: WARNING - High latency may affect sync\n";
            }
          }
        } break;
        ...
        case 10:  // Toggle Metrics Logging
        {
          ...
          if (g_metrics_logger->isEnabled()) {
            ...
            std::cout << "[Main] Metrics logging started. Data will be saved "
                         "to Desktop on "
                         "pipeline stop.\n";
          }
        } break;
