...
void SensorFusion::run() {
  ...
  cv::Mat egoFrameInitialState =
      (cv::Mat_<float>(5, 1) << 0, 0, 0, 0, 0); // Replace
  ...
  if (auto radarTargets = FrontRadarQueue.try_pop()) {
    if (radarTargets.has_value()) {
      ...
      if (frontTrack.previous_time_ns != 0) {
        dt = adas::Clock::ns_to_sec(radarTargets->h.t_ingest_ns -
                                    frontTrack.previous_time_ns);
      }
      frontTrack.previous_time_ns = radarTargets->h.t_ingest_ns;
      if (avg_speed <= 0.20 && avg_speed >= -0.02 && final_range <= 2.42 &&
          final_range >= 2.38) { // TODO redo this based off actual sensor data
        ...
        if (!frontTrack.kf_initialized) {
          frontTrack = adas::Track(
              (cv::Mat_<float>(4, 1) << final_range, 0.0f, avg_speed, 0.0f));
          std::cout << "[SensorFusion] Front Track Initialized:"
                    << frontTrack.getPrediction().reshape(1, 1) << std::endl;
        } else {
          cv::Mat estimate = frontTrack.update(
              (cv::Mat_<float>(4, 1) << final_range, 0.0f, avg_speed, 0.0f),
              dt);
          std::cout << "[SensorFusion] Front Track Updated:"
                    << estimate.reshape(1, 1) << std::endl;
        }
        std::cout << "Avg Speed: " << avg_speed
                  << " m/s, Final Range: " << final_range << " m" << std::endl;
      }
    }
  }
}
...
void SensorFusion::updateEgoFrame() {
  ...
  // Some random stuff that can kinda get speed and distance from imu since we
  // don't have gps
  if (auto imuSample = IMUQueue.try_pop()) {
    ...
    if (imuSample.has_value()) {
      ...
      if (egoFrame.previous_time_ns != 0) {
        dt = adas::Clock::ns_to_sec(imuSample->h.t_ingest_ns -
                                    egoFrame.previous_time_ns);
      }
      speed_x = egoFrame.getPrediction().at<float>(2, 2) +
                (imuSample->acc_mps2[0] * dt); // Replace
      egoFrame.previous_time_ns = imuSample->h.t_ingest_ns;
      distance_x =
          egoFrame.getPrediction().at<float>(0, 0) + (speed_x * dt); // Replace
      egoFrame.update(
          (cv::Mat_<float>(4, 1) << distance_x, 0.0f, speed_x, 0.0f), dt);

      std::cout << "[SensorFusion] EgoFrame updated:"
                << egoFrame.getPrediction().reshape(1, 1) << std::endl;
    }
  }
}
...
void SensorFusion::detectFCW(cv::Mat ef, cv::Mat track) {
  ...
  // stope_time = vehicle_speed / (friction_coefficient * gravity) +
  // reaction_time
