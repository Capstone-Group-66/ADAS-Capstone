...
void MetricsLogger::logFrame(uint64_t timestamp_ms, uint32_t frame_id, double inference_ms,
                             float ttc_s, float range_m, bool fcw_triggered,
    ...
    entries_.push_back(
        {timestamp_ms, frame_id, inference_ms, ttc_s, range_m, fcw_triggered, e2e_latency_ms});
}
...
bool MetricsLogger::dumpToCSV(const std::string &filepath) {
    ...
    // Write CSV header
    file << "timestamp_ms,frame_id,inference_ms,ttc_s,range_m,fcw_triggered,e2e_latency_ms\n";

    // Write data rows
    for (const auto &entry : entries_) {
        file << entry.timestamp_ms << "," << entry.frame_id << "," << std::fixed
             << std::setprecision(2) << entry.inference_ms << "," << std::fixed
             << std::setprecision(3) << entry.ttc_s << "," << std::fixed << std::setprecision(2)
             << entry.range_m << "," << (entry.fcw_triggered ? 1 : 0) << "," << std::fixed
             << std::setprecision(2) << entry.e2e_latency_ms << "\n";
    }

    file.close();
    std::cout << "[MetricsLogger] Wrote " << entries_.size() << " entries to: " << filepath << "\n";
    return true;
}
