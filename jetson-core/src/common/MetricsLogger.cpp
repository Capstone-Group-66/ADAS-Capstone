...
bool MetricsLogger::isEnabled() const {
...
bool MetricsLogger::dumpToCSV(const std::string& filepath) {
    ...
    for (const auto& entry : entries_) {
        file << entry.timestamp_ms << ","
             << entry.frame_id << ","
             << std::fixed << std::setprecision(2) << entry.inference_ms << ","
             << std::fixed << std::setprecision(3) << entry.ttc_s << ","
             << std::fixed << std::setprecision(2) << entry.range_m << ","
             << (entry.fcw_triggered ? 1 : 0) << ","
             << std::fixed << std::setprecision(2) << entry.e2e_latency_ms << "\n";
    }
