...
Config ConfigLoader::loadConfig(const std::string& path) {
    ...
    while (std::getline(stream, line)) {
        ...
        // Time config
        if (key == "fusion_hz") config.time.fusion_hz = std::stoi(value);
        else if (key == "max_skew_ms") config.time.max_skew_ms = std::stoi(value);
        else if (key == "buffer_ms") config.time.buffer_ms = std::stoi(value);
        else if (key == "late_drop_ms") config.time.late_drop_ms = std::stoi(value);
        else if (key == "warmup_ticks") config.time.warmup_ticks = std::stoi(value);

        // Camera config
        else if (key == "width") config.cameras.width = std::stoi(value);
        else if (key == "height") config.cameras.height = std::stoi(value);
        else if (key == "target_fps") config.cameras.target_fps = std::stoi(value);
        else if (key == "use_mjpeg") config.cameras.use_mjpeg = (value == "true");

        // Network config
        else if (key == "port") config.network.port = std::stoi(value);
        else if (key == "latency_correction_ms") config.network.latency_correction_ms = std::stoi(value);
        else if (key == "reconnect_timeout_ms") config.network.reconnect_timeout_ms = std::stoi(value);

        // Radar config
        else if (key == "baud_rate") config.front_radar.baud_rate = std::stoi(value);
        else if (key == "poll_timeout_ms") config.front_radar.poll_timeout_ms = std::stoi(value);

        // IMU config
        else if (key == "rate_hz") config.imu.rate_hz = std::stoi(value);
        else if (key == "use_uart") config.imu.use_uart = (value == "true");
    }
