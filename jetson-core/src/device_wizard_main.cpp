...
void printUsage(const char *program_name) {
  std::cout
      ...
      << "  --no-preview            Skip camera preview windows during "
         "registration\n"
      << "\nCamera Calibration:\n"
      << "  --calibrate             Run camera calibration after registration\n"
      << "  --recalibrate           Force recalibration even if calibration "
         "files exist\n"
...
int main(int argc, char *argv[]) {
  ...
  if (run_calibration) {
    ...
    adas::CameraCalibrator calibrator(cv::Size(pattern_w, pattern_h),
                                      square_size);
    ...
    for (const auto &[mount, device_path] : hw_map.mappings) {
      ...
      bool exists =
          adas::CameraCalibrator::calibrationExists(mount, calibration_dir);
      ...
      // Run calibration
      auto result =
          calibrator.runCalibration(device_path, mount, calibration_dir);
