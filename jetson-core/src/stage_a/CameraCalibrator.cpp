...
CameraCalibrator::CameraCalibrator(cv::Size pattern_size, double square_size_m,
                                   int num_images)
    : pattern_size_(pattern_size), square_size_m_(square_size_m),
      num_images_(num_images) {}

std::optional<CameraIntrinsics>
CameraCalibrator::runCalibration(const std::string &device_path, Mount mount,
  ...
  std::cout << "  Pattern: " << pattern_size_.width << "x"
            << pattern_size_.height << " inner corners\n";
  ...
  std::cout << "\n  Computing calibration from " << images.size()
            << " images...\n";
  ...
  if (saveCalibration(calib.value(), output_path)) {
    ...
    std::cout << "  RMS reprojection error: " << calib->rms_error
              << " pixels\n";
...
std::vector<cv::Mat>
CameraCalibrator::captureCalibrationImages(const std::string &device_path) {
  ...
  std::string window_name =
      "Calibration - SPACE to capture, Q when done, ESC to cancel";
  ...
  while (true) {
    ...
    // Draw status text
    std::string status = "Captured: " + std::to_string(captured_count) + "/" +
                         std::to_string(num_images_);
    if (found) {
    ...
    cv::putText(display, status, cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX,
...
bool CameraCalibrator::findCorners(const cv::Mat &image,
...
CameraCalibrator::computeCalibration(const std::vector<cv::Mat> &images) {
  ...
  std::cout << "  Using " << object_points.size()
...
std::optional<CameraIntrinsics>
CameraCalibrator::loadCalibration(const std::string &input_path) {
...
bool CameraCalibrator::calibrationExists(Mount mount,
                                         const std::string &calib_dir) {
  return std::filesystem::exists(getCalibrationPath(mount, calib_dir));
}

std::string CameraCalibrator::getCalibrationPath(Mount mount,
                                                 const std::string &calib_dir) {
