...
bool BSDReceiver::start() {
    ...
    if (zmq_connect(left_socket_, left_uri.c_str()) != 0) {
        std::cerr << "[BSDReceiver] Failed to connect to left radar socket at " << left_uri << "\n";
        return false;
    }

    // Create Right Socket
    right_socket_ = zmq_socket(context_, ZMQ_PULL);
    if (!right_socket_) return false;
    ...
    if (zmq_connect(right_socket_, right_uri.c_str()) != 0) {
        std::cerr << "[BSDReceiver] Failed to connect to right radar socket at " << right_uri << "\n";
        return false;
    }
    ...
    std::cout << "[BSDReceiver] Started successfully. Left=" << left_uri << ", Right=" << right_uri << "\n";
...
void BSDReceiver::processFailsafe(uint64_t now_ms) {
    ...
    if (left_state_.load() && (now_ms - left_last_rx_ms_ > TIMEOUT_MS)) {
        ...
        std::cerr << "[BSDReceiver] Watchdog Left Timeout (>300ms). Resetting state.\n";
    }
    
    // Failsafe Right
    if (right_state_.load() && (now_ms - right_last_rx_ms_ > TIMEOUT_MS)) {
        right_state_.store(false, std::memory_order_relaxed);
        std::cerr << "[BSDReceiver] Watchdog Right Timeout (>300ms). Resetting state.\n";
    }
}

void BSDReceiver::receiveLoop() {
    zmq_pollitem_t items[] = {
        { left_socket_, 0, ZMQ_POLLIN, 0 },
        ...
        if (items[0].revents & ZMQ_POLLIN) {
            if (zmq_msg_recv(&msg, left_socket_, 0) > 0) {
                ...
                if (msg_len >= sizeof(BSDHeader)) {
                    ...
                    if (header->magic == 0x50493034 && header->version == 0x0100 && header->msg_type == 0x0002) {
                        if (header->payload_size == 1 && msg_len >= sizeof(BSDHeader) + 1) {
                            ...
                            if (state == 0 || state == 1) {
                                ...
                                if (left_last_seq_ > 0 && header->sequence != left_last_seq_ + 1) {
        ...
        if (items[1].revents & ZMQ_POLLIN) {
            if (zmq_msg_recv(&msg, right_socket_, 0) > 0) {
                ...
                if (msg_len >= sizeof(BSDHeader)) {
                    ...
                    if (header->magic == 0x50493034 && header->version == 0x0100 && header->msg_type == 0x0003) {
                        if (header->payload_size == 1 && msg_len >= sizeof(BSDHeader) + 1) {
                            uint8_t state = msg_data[sizeof(BSDHeader)];
                            if (state == 0 || state == 1) {
                                ...
                                // Sequence gap tracking
                                if (right_last_seq_ > 0 && header->sequence != right_last_seq_ + 1) {
