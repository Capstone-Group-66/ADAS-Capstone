...
void NetworkReceiver::radarLThread() {
    ...
    while (running_.load()) {
        ...
        if (!validateHeader(header) ||
            static_cast<MessageType>(header.msg_type) != MessageType::REAR_RADAR_L) {
            stats_.errors++;
            continue;
        }

        // Check drops
        if (stats_.radar_l_packets > 0 && header.sequence != last_radar_l_seq_ + 1) {
...
void NetworkReceiver::radarRThread() {
    ...
    while (running_.load()) {
        ...
        if (!validateHeader(header) ||
            static_cast<MessageType>(header.msg_type) != MessageType::REAR_RADAR_R) {
            stats_.errors++;
            continue;
        }

        if (stats_.radar_r_packets > 0 && header.sequence != last_radar_r_seq_ + 1) {
...
void NetworkReceiver::imuThread() {
    ...
    while (running_.load()) {
        ...
        std::memcpy(&imu_payload, buffer.data() + sizeof(PiMessageHeader), sizeof(imu_payload));
...
NetworkReceiver::discoverDevices(const std::string& pi_ip, int timeout_ms) {
    ...
    if (!validateHeader(resp_header) ||
        static_cast<MessageType>(resp_header.msg_type) != MessageType::DISCOVERY_RSP) {
    ...
    for (int i = 0; i < resp.num_devices; i++) {
        ...
        dev.serial = std::string(info.serial, strnlen(info.serial, sizeof(info.serial)));
