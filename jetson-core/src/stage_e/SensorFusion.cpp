...
int SensorFusion::findRadarMatch(float cam_azimuth,
                                 const RadarTargets &radar) const {
  // For 1D radar (OPS243-A), we ignore azimuth and just check if we have any
  // targets Return the closest target (smallest range) if there are any
  if (radar.targets.empty()) {
    return -1;
  }

  // For a 1D forward-facing radar, assume the closest target is the one of
  // interest
  int closest_idx = 0;
  ...
  // Only match if camera detection is reasonably centered (within ~30 degrees
  // of center) This is the "boresight cone" where the 1D radar is also pointing
