...
struct TrackedObject {
    ...
    TrackedObject() : id(0), first_seen_ns(0), last_seen_ns(0), hit_count(0), miss_count(0) {}
};
...
class ObjectTracker {
  public:
    struct Config {
        ...
        Config()
            : iou_threshold(0.3f), max_miss_count(30) // ~1 second at 30 FPS
              ,
              min_hit_count(3) {}
    };
  ...
  private:
    ...
    std::vector<std::pair<int, int>> matchDetections(const std::vector<Det> &detections);
